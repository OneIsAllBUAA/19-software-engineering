...
def api_submit_task(request):
    ...
    current_user = models.User.objects.filter(name='username').first()
...
def api_my_task(request):
    print(request.body)
    req = simplejson.loads(request.body)
    user = models.User.objects.filter(name=req['username']).first()
    # favorite
    f_tasks = json.loads(serializers.serialize("json",user.favorite_tasks.all()))
    # grabbed
    g_tasks = json.loads(serializers.serialize("json",models.TaskUser.objects.filter(user=user, is_grabbed=True).values("task")))
    # released
