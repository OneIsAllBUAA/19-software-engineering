package com.oneisall.views.MySearchView;
...
public class SearchView extends LinearLayout {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        int defaultColor = context.getResources().getColor(R.color.text_black); // 默认颜色 = 灰色
        ...
        // 搜索框颜色
        int defaultColor2 = context.getResources().getColor(R.color.colorWhite); // 默认颜色 = 白色
...
//                    Toast.makeText(context, "需要搜索的是" + et_search.getText(), Toast.LENGTH_SHORT).show();
                    ...
                    data = data.replace("'","");
                    boolean hasData = hasData(data.trim());
                    // 3. 若存在，则不保存；若不存在，则将该搜索字段保存（插入）到数据库，并作为历史搜索记录
                    if (!data.trim().equals("") && !hasData) {
                        insertData(data.trim());
                        queryData("");
                    }
                }
                return false;
